  ...
  describe '#diff' do
    describe 'single vals' do
      it "tracks value changes correctly" do
        ...
        evts.first.event_type.should == Basilik::Root.evt_value
      ...
      it "tracks adds correctly" do
        ...
        evts = prev.diff( curr )
        evts.should have(1).item
        evts.first.event_type.should == Basilik::Root.evt_child_added
        evts.first.ref.should == 'a'
      end    
      ...
      it "tracks deletes correctly" do
        ...
        evts = prev.diff( curr )
        evts.should have(1).item
        evts.first.event_type.should == Basilik::Root.evt_child_removed
        evts.first.ref.should == 'a'
      end
    end
